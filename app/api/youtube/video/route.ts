...
// ─── YouTube Video Info ───────────────────────────────────────────────────────
// Strategy: Scrape m.youtube.com (Mobile Web) - works on Vercel datacenter IPs
// without poToken, cookies auth, or 3rd-party APIs.
// Scans ALL occurrences of ytInitialPlayerResponse to find the real JSON block
// (YouTube injects a dummy "= null" entry first to mislead basic scrapers).
...
function parseFormat(fmt: any, type: "adaptive" | "muxed"): StreamFormat | null {
  ...
  const qualityLabel =
    fmt.qualityLabel ||
    (fmt.audioQuality
      ? fmt.audioQuality.replace("AUDIO_QUALITY_", "").toLowerCase()
      : "");
...
function extractPlayerResponse(html: string): any | null {
  ...
  while (searchStart < html.length) {
    ...
    for (
      let i = startBrace;
      i < Math.min(html.length, startBrace + 6_000_000);
      i++
    ) {
      const ch = html[i];
      if (esc) {
...
async function fetchVideoInfo(videoId: string): Promise<VideoInfo> {
  ...
  // Strategy 1: MWEB scrape (best for datacenter IPs — no bot token needed)
  try {
    playerData = await fetchFromMWEB(videoId);
    console.log(
      `[video] MWEB OK | formats:${playerData?.streamingData?.formats?.length || 0} adaptive:${playerData?.streamingData?.adaptiveFormats?.length || 0}`
    );
  } catch (e) {
    console.warn("[video] MWEB failed:", (e as Error).message);
  }

  // Strategy 2: TVHTML5 InnerTube (good fallback, no auth for most videos)
  if (!playerData?.streamingData?.adaptiveFormats?.length) {
    try {
      ...
      if (tv?.streamingData?.adaptiveFormats?.length) {
        // Merge metadata from MWEB if available, streams from TV
        playerData = playerData
          ? { ...tv, videoDetails: playerData.videoDetails || tv.videoDetails }
          : tv;
        console.log(
          `[video] TVHTML5 OK | adaptive:${tv.streamingData.adaptiveFormats.length}`
        );
      }
    } catch (e) {
      console.warn("[video] TVHTML5 failed:", (e as Error).message);
    }
  }

  if (!playerData?.videoDetails) {
    console.warn("[video] All strategies failed, using oEmbed metadata");
